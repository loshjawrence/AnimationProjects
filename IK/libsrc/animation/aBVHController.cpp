...
void BVHController::update(double time)
{
	...
	//update local2parents orientations, fixed bone length so no need to update translations
	for (int id = 0; id < skeleton->getNumJoints(); id++) {
		mat3 new_orientation = mMotion[id].getCachedValue(time).ToRotation();
		skeleton->getJointByID(id)->setLocalRotation(new_orientation);
	}
	skeleton->update();//update local2globals
}
