...
quat quat::Slerp(const quat& q0, const quat& q1, double u)
{
   ...
   //if u = 0 then theta = 0, if u = 1 then theta = omega 
    float omega = acos( quat::Dot(q0, q1) );
    float theta = u * omega;
    float sin_omega = sin( omega );
    
    float q0_weight = sin( (1-u) * omega ) / sin_omega;
    float q1_weight = sin(    u  * omega ) / sin_omega;
    quat q = q0_weight * q0 + q1_weight * q1;
...
quat quat::Scubic(const quat& b0, const quat& b1, const quat& b2, const quat& b3, double u)
{
    ...
    //CONTROL POINTS ARE LVL 0 of SYSTOLIC ARRAY
    //LVL1 OF SYSTOLIC ARRAY
    b01 = quat::Slerp(b0,b1,u);
    b11 = quat::Slerp(b1,b2,u);
    b12 = quat::Slerp(b2,b3,u);
    //LVL 2 OF SYSTOLIC ARRAY
    b02 = quat::Slerp(b01,b11,u);
    b12 = quat::Slerp(b11,b12,u);
    //LVL 3 OF SYSTOLIC ARRAY (result)
...
void quat::ToAxisAngle (vec3& axis, double& angleRad) const
{
    ...
    float thetaover2 = acos(mQ[VW]);
    angleRad = 2 * thetaover2;

    float sinhalf = sin(thetaover2);
    axis[0] = mQ[VX] / sinhalf;
    axis[1] = mQ[VY] / sinhalf;
    axis[2] = mQ[VZ] / sinhalf;
...
void quat::FromAxisAngle (const vec3& axis, double angleRad)
{
    ...
    float sinhalf = sin(angleRad / 2);
    mQ[VX] = axis[0] * sinhalf;
    mQ[VY] = axis[1] * sinhalf;
    mQ[VZ] = axis[2] * sinhalf;
